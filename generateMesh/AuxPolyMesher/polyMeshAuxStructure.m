...
%
%   [30/4/2018], add nuEdge, see the details in the following %>23.
...
%- nuEdges
nuEdges = zeros(Nedges,2); % [Nedges x 2]
    %>23 the unit normal vector of all edges, and here, we set e = T1 \intersect T2, and T1_index > T2_index,
...
for ii = 1:Nedges
    ...
    %- nuEdges
    elem1_index = edge2elem(ii,1);
    elem2_index = edge2elem(ii,2);
    local_e1 = edge2elem(ii,3);
    local_e2 = edge2elem(ii,4);
    elemLarge = max(elem1_index, elem2_index);
    if elemLarge == elem1_index
        eIndx_elemLarge = local_e1;
    else
        eIndx_elemLarge = local_e2;
    end 
    nuEdges(ii,:) = nuEdge0Elem{elemLarge}(:,eIndx_elemLarge); % [Nedges x 2]
end % for ii = 1: Nedges
...
g.hElem = hElem; 
    ...
    %> the max distance between centroid(xing xin) and the element vertices.
    %> And hElem is used in the local bases, replacing the diameters used in the local bases.
g.nuEdges = nuEdges; % [Nedges x 2]
    %>23, [Nedges x 2], the unit normal vector of all edges, and here, we set e = T1 \intersect T2, and T1_index > T2_index,
    %> then, the unit normal vector of e, is from T1 to T2 (T1-->T2).
