function [L2err, H1err] = hhoL2H1_Terror(P,Ph,meshInfo,Gaussformula2D,Tbasesk)
%
...
for CurrElem = 1:Nelems
    ...
    PhOnElem = Ph((CurrElem-1)*NTbases+1 : CurrElem*NTbases,1); % [NTbases_trial x 1]
    %<<-- End Part I ---------------------------------------------------------------------------------------
    ...
    for nt = 1:(size(coordTri0Elem,1)/3)
        ...
        wPb = bsxfun(@times, phyGweights, Pb); % trialPb, trialPbx, trialPby, [Npoints x NTbases_trial]      
    ...
    Pt_h = mat_temp\valueP_temp; % the L2 projection of exact solution on Elem.
    ...
    for nt = 1:(size(coordTri0Elem,1)/3)
        ...
        % get the bases values on quad points
        [Pb, Pbx, Pby] = localBases2D(elem_xT, elem_yT, elem_hT, ...
            phyGpoints(:,1), phyGpoints(:,2), Tbasesk);% trialPb, trialPbx, trialPby, [Npoints x NTbases_trial]      
        
        L2err = L2err + phyGweights'*(Pb*Pt_h - Pb*PhOnElem).^2;
        H1err = H1err + phyGweights'*(Pbx*Pt_h-Pbx*PhOnElem).^2 ...
            + phyGweights'*(Pby*Pt_h-Pby*PhOnElem).^2;
        
        projection_Pth = projection_Pth + phyGweights'*(Pb*Pt_h ).^2;
        projection_Pthx = projection_Pthx + phyGweights'*(Pbx*Pt_h ).^2;
        projection_Pthy = projection_Pthy + phyGweights'*(Pby*Pt_h ).^2;
    end % for nt
